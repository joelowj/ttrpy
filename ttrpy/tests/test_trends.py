...
class TestTrendIndicators(unittest.TestCase):
    ...
    def setUp(self):
        self.wdf = pd.read_csv("examples/weekly_MSFT.csv") \
                    .sort_values(by="timestamp") \
                    .reset_index(drop=True)
        self.ddf = pd.read_csv("examples/daily_MSFT.csv") \
                     .sort_values(by="timestamp") \
                     .reset_index(drop=True)

    def test_simple_moving_average(self):
        self.wdf = sma(self.wdf, 'open', 'sma', 10).dropna() \
                                                 .reset_index(drop=True)
    ...
    def test_exponential_moving_average(self):
        self.wdf = ema(self.wdf, 'open', 'ema', 10).dropna() \
                                                 .reset_index(drop=True)
    ...
    def test_weighted_moving_average(self):
        self.wdf = wma(self.wdf, 'open', 'wma', 10).dropna() \
                                                 .reset_index(drop=True)
